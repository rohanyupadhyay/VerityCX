...
- [x] T049 CRITICAL Add a conventional leading HTML documentation comment to `specs/001-acquire-tau3-banking/checklists/comprehensive.md` that accurately states the checklist's purpose and maintained-file role per Constitution II (contradicts)
- [x] T050 CRITICAL Format `specs/001-acquire-tau3-banking/checklists/requirements.md`, add both Feature 001 checklist files to the documented mdformat verification set and `.github/workflows/quality.yml`, and prove the automated Markdown gate checks them per Constitution V (contradicts)
- [x] T051 CRITICAL Make `_run_git()` hermetic against repository/worktree/index overrides, injected `GIT_CONFIG_*` entries, system/global URL rewrites, credential helpers, and askpass/credential environment controls while preserving the fixed executable argument-list boundary; add hostile-environment tests proving acquisition cannot use an alternate source or credentials per FR-002 and FR-016 (contradicts)
- [x] T052 Make both public scripts executable by absolute path from an unrelated current working directory under the locked project environment, without `PYTHONPATH` or `sys.path` modification, and add subprocess-level setup/check/inspection tests that prove stable exits and output from that location per FR-001, FR-012, and FR-015 (partial)
- [x] T053 Replace `Path.rename()` promotion with an atomic same-filesystem no-replace operation that cannot replace a destination created inside the final promotion window on POSIX or Windows, preserve the competing destination and owned cleanup state, and add cross-platform race tests per FR-010 and US2/AC8 (contradicts)
- [x] T054 Reject non-standard JSON constants including `NaN`, `Infinity`, and `-Infinity` at the typed database boundary and add sanitized validation and inspection tests proving they cannot be accepted as JSON numbers per FR-007 (partial)
- [x] T055 Centralize setup and inspection diagnostic rendering so every expected failure emits only a declared category, the applicable sanitized configured or owned path, required safe expected/detected fields, and a non-destructive recovery action; map Git prerequisite failures to `git-unavailable`, report the actual retained path on cleanup failure, and add exact CLI tests per FR-011 (partial)
- [x] T056 Complete the checked-but-undercovered network-independent test obligations with closed-schema and resolved-escape cases, every required documents/database/tasks missing-empty-wrong-kind-unreadable-link-junction-special-escape variant, invalid existing default/check offline preservation, wrong tag binding, failures before/during/after promotion, cache and target permission/object cases, and inspection state/count/shape-change coverage per plan: network-independent test strategy (partial)
- [ ] T057 Update `.github/workflows/quality.yml` to record the matrix runner label and actual hosted image/version and to fail the measured first-acquisition gate at 600 seconds or more; then execute and retain one clean passing attempt on `ubuntu-latest`, `windows-latest`, and `macos-latest`, complete T048's final official smoke and SC-007/SC-008 evidence against the resulting candidate, and record all required logs per SC-001 and SC-002 (partial)

## Phase 8: Convergence

- [x] T058 CRITICAL Preserve the lexical identity of `.cache/`, the configured checkout, and the configured documents, database, and tasks paths until each boundary has been classified without following links; reject in-project and escaping symbolic links, junctions, and reparse points through the complete setup, `--check`, and inspection flows; and add capability-aware end-to-end preservation tests rather than helper-only mocked classifications per FR-006-FR-010 and plan: validation rules (contradicts)
- [ ] T059 Measure and record the network-independent first-acquisition duration from the setup operation/process start through validated promotion and successful exit on every required matrix job, excluding pytest collection and fixture-construction time from the SC-001 interval per SC-001 (partial)
- [ ] T060 Complete the still-open T048/T057 acceptance execution, retain one clean passing `ubuntu-latest`, `windows-latest`, and `macos-latest` attempt with the required image/tool/command/duration evidence, perform the one-environment official-upstream smoke with independent safe count/shape comparison, repeat and record the SC-007 tracked-content audit, complete the documentation-only SC-008 review, and close the prior gates only when every required record passes per SC-002 and SC-006-SC-008 (missing)
