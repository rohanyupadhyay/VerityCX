...
- **SC-007**: After setup, `git ls-files -- .cache/tau3-bench/` returns zero paths, `git check-ignore -v .cache/tau3-bench/` identifies the intended project ignore rule, and review of the Feature 001 tracked change set finds no file or file content copied from the acquired upstream checkout.
