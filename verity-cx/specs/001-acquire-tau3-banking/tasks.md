...
- [ ] T006 [P] Create schema version 1 with the exact MIT licence, canonical repository URL `https://github.com/sierra-research/tau2-bench.git`, `v1.0.1` tag, `fc0055dc4e0a316c3f83133267fbd6faaa770992` SHA, and four project-root-relative paths in `config/tau3-bench.toml`
...
- [ ] T015 [US1] Implement `GitCheckoutState`, `BankingDataState`, exact standalone-checkout identity validation, clean-status validation, required-path validation, and sanitized deterministic error categories in `src/veritycx/data_sources/tau3.py` to satisfy T012-T013
- [ ] T016 [US1] Implement `SetupExecution`, the immutable setup result, cooperative lock acquisition, unique `tau3-bench-staging-` ownership, argument-list clone, staged validation, destination recheck, non-replacing rename promotion, post-promotion validation, and exact-owned-state cleanup in `src/veritycx/data_sources/tau3.py`
- [ ] T017 [US1] Implement the project-root-resolving default acquisition command and stable success/error translation in `scripts/setup_tau3_data.py` with no production source, revision, destination, or config options
- [ ] T018 [P] [US1] Document prerequisites, the single acquisition command, installed success fields, external-cache boundary, first-install failures, and verification entry points in `README.md`
- [ ] T019 [P] [US1] Document the fixed schema, authority, exact pin, path-containment rules, production-override prohibition, and test-injection boundary in `config/README.md`
- [ ] T020 [P] [US1] Document the package purpose, public imports, root-resolution invariant, dependency boundary, setup usage, and package-level test command in `src/veritycx/README.md`
- [ ] T021 [US1] Verify `git ls-files -- .cache/tau3-bench/` returns zero paths, `git check-ignore -v .cache/tau3-bench/` resolves to the intended `.gitignore` rule, and review of the Feature 001 tracked change set finds no file or file content copied from the acquired checkout; record any ignore-rule correction in `.gitignore`
...
- [ ] T030 [P] [US2] Document typed setup interfaces, configuration/Git/data layers, validation order, transaction state machine, security invariants, and failure modes in `src/veritycx/data_sources/README.md`
...
- [ ] T037 [P] [US3] Add the inspection command, safe output contract, third-party documentation link, allow-list/deny-list summary, and explicit exclusions to `README.md`
- [ ] T038 [P] [US3] Add inspection usage, output restrictions, exit behavior, and validation-first/no-mutation guarantees to `scripts/README.md`
- [ ] T039 [P] [US3] Document local-Git fixtures, generated canaries, platform permission injection, test commands, and forbidden real-network/upstream-content use in `tests/data_sources/README.md`
- [ ] T040 [P] [US3] Add inspection interfaces, safe result fields, serialization restrictions, default-deny policy, and non-disclosure failure modes to `src/veritycx/data_sources/README.md`
...
- [ ] T041 Audit and complete module/callable docstrings, explicit types, rationale comments, and public exports in `src/veritycx/__init__.py`, `src/veritycx/data_sources/__init__.py`, `src/veritycx/data_sources/tau3.py`, `scripts/setup_tau3_data.py`, `scripts/inspect_tau3_banking_data.py`, and `tests/data_sources/test_tau3.py`, plus conventional leading comments in `pyproject.toml`, `.gitignore`, `config/tau3-bench.toml`, `README.md`, `config/README.md`, `scripts/README.md`, `src/veritycx/README.md`, `src/veritycx/data_sources/README.md`, `tests/data_sources/README.md`, `docs/data/tau3-banking.md`, and `THIRD_PARTY_NOTICES.md`, while preserving the `.python-version` and `uv.lock` exemptions
